...
<header>
		...
		<div class="bord_header">
			<div class="this_board">
				<button type="button" class="board_button" id="board_button"
					onclick="location.href='sports?sports=baseball'">야구 게시판</button>
			</div>
			...
			<div id="div_list_home">
				...
				<div>
					<button type="button" id="button_list"
						onclick="location.href='sports?sports=baseball'">목록</button>
				</div>
				<div>
			...
			<c:forEach var="cment" items="${cment}">
				<div id="reple_output">
					...
					<c:if test="${logID == cment.id || logID == 'admin'}">
						<a href="comment_delelte?seq=${cment.seq}&root=${cment.root}?sports=baseball"
	...
	<div align="center">
		...
		<c:if test="${pageMaker.prev && pageMaker.sPageNo>1}">
			<a href="baseball_Detail${pageMaker.searchmakeQuery(1)}&sports=baseball&seq=${Detail.seq}">처음</a>&nbsp;
		    <a href="baseball_Detail${pageMaker.searchmakeQuery(pageMaker.sPageNo -1)}&sports=baseball&seq=${Detail.seq}">이전</a>&nbsp;
      </c:if>
		...
		<c:forEach var="i" begin="${pageMaker.sPageNo}"
			...
			<c:if test="${i!=pageMaker.cri.currPage}">
				<a href="baseball_Detail${pageMaker.searchmakeQuery(i)}&sports=baseball&seq=${Detail.seq}">${i}</a>
			</c:if>
		...
		<c:if test="${pageMaker.next && pageMaker.ePageNo >0}">
			<a href="baseball_Detail${pageMaker.searchmakeQuery(pageMaker.ePageNo+1)}&sports=baseball&seq=${Detail.seq}">&nbsp;다음</a>&nbsp;
		<a href="baseball_Detail${pageMaker.searchmakeQuery(pageMaker.lastPageNo)}&sports=baseball&seq=${Detail.seq}">마지막</a>
		</c:if>
	</div>
	<!-- 댓글 페이징 end -->
