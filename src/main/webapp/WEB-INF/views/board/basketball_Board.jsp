...
<link rel="stylesheet" type="text/css"
	href="resources/css/basketball_board.css">
...
<body style="margin: 0 auto;">
	...
	<div class="bord_header">
		<div class="this_board">
			<button type="button" class="board_button" id="board_button"
				onclick="location.href='basketball_Board?sports=basketball'">농구
				게시판</button>
		</div>
		...
		<div id="searchBar">
			<select name="searchType" id="searchType" hidden="hidden">
				<option value="basketball"
					...
					농구</option>
			</select> <input type="text" class="basketballsearch" id="keyword"
	...
	<div align="center">
		...
		<c:if test="${pageMaker.prev && pageMaker.sPageNo>1}">
			<a href="sports${pageMaker.searchmakeQuery(1)}&sports=basketball">처음</a>&nbsp;
		    <a href="sports${pageMaker.searchmakeQuery(pageMaker.sPageNo -1)}&sports=basketball">이전</a>&nbsp;
      </c:if>
		...
		<c:forEach var="i" begin="${pageMaker.sPageNo}"
			...
			<c:if test="${i!=pageMaker.cri.currPage}">
				<a href="sports${pageMaker.searchmakeQuery(i)}&sports=basketball">${i }</a>
			</c:if>
		...
		<c:if test="${pageMaker.next && pageMaker.ePageNo >0}">
			<a href="sports${pageMaker.searchmakeQuery(pageMaker.ePageNo+1)}&sports=basketball">&nbsp;다음</a>&nbsp;
		<a href="sports${pageMaker.searchmakeQuery(pageMaker.lastPageNo)}&sports=basketball">마지막</a>
		</c:if>
	</div>
